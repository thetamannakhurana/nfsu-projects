...
export default function FacultyGuidancePage() {
  ...
  const RequestCard = ({ req }: { req: Request }) => {
    const sem = req.batch_start_year && req.batch_end_year ? getCurrentSem(req.batch_start_year, req.batch_end_year) : null
      ...
      <div className="px-6 py-5 border-b border-gray-100 last:border-0">
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
          <div className="flex-1">
            ...
            {/* Report + Plagiarism section for accepted requests */}
            {req.status === 'accepted' && (
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center gap-3">
                      <a href={req.report_url} target="_blank" rel="noopener noreferrer"
                        className="flex items-center gap-2 text-sm text-nfsu-blue hover:underline font-medium">
                        📥 Download Report — {req.report_filename}
                      </a>
                    ...
                    {req.plagiarism_score !== null && req.plagiarism_score !== undefined ? (
                      <div className={`rounded-lg px-3 py-2 text-xs border ${
                        ...
                        ✅ Plagiarism result submitted: <strong>{req.plagiarism_score}%</strong>
                      ...
                      <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-3">
                        ...
                        {plagiarismSuccess === req.id && (
                          <div className="bg-green-50 border border-green-200 text-green-700 text-xs rounded-lg px-3 py-2">✅ Plagiarism result submitted!</div>
                        )}
                        ...
                        <div>
                          ...
                          <textarea value={pf.remarks} onChange={e => setPF(req.id, 'remarks', e.target.value)}
                            rows={2} placeholder="Add any notes about the plagiarism check..."
          ...
          {/* Respond buttons — only for pending/held */}
              ...
              {isResponding ? (
                <div className="w-64">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <button onClick={() => { setResponding(null); setNote('') }} className="w-full btn-outline text-xs py-2">Cancel</button>
                  </div>
    ...
    <div className="min-h-screen bg-nfsu-offwhite">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
          ...
          {pending.length === 0 ? <div className="p-10 text-center text-gray-400">No pending requests</div>
        ...
        {/* Accepted — with report/plagiarism */}
        {accepted.length > 0 && (
          <div className="bg-white rounded-2xl border border-green-200 overflow-hidden">
            <div className="px-6 py-4 border-b border-green-100 bg-green-50/50">
              <h2 className="font-heading font-semibold text-green-800">✅ Accepted — My Students ({accepted.length})</h2>
              <p className="text-xs text-green-500 mt-0.5">Download reports and submit plagiarism check results</p>
            </div>
